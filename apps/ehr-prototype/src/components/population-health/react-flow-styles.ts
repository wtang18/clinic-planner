/**
 * React Flow CSS Injection
 *
 * Injects @xyflow/react styles via a DOM <style> tag instead of a bare CSS import.
 * This avoids the `import.meta` error caused by Expo/Metro's CSS processing pipeline,
 * which doesn't support `import '*.css'` in non-module script contexts.
 *
 * The CSS is from @xyflow/react/dist/style.css (v12).
 * Call `injectReactFlowStyles()` once before rendering <ReactFlow />.
 */

let injected = false;

export function injectReactFlowStyles(): void {
  if (injected) return;
  if (typeof document === 'undefined') return; // SSR guard

  const style = document.createElement('style');
  style.setAttribute('data-react-flow', '');
  style.textContent = REACT_FLOW_CSS;
  document.head.appendChild(style);
  injected = true;
}

// ---------------------------------------------------------------------------
// Inlined CSS from @xyflow/react/dist/style.css
// ---------------------------------------------------------------------------

const REACT_FLOW_CSS = `
.react-flow {
  direction: ltr;
...
.react-flow__pane.selection .react-flow__panel { pointer-events: none; }
...
.react-flow__controls.horizontal .react-flow__controls-button:last-child { border-right: none; }
...
.react-flow__resize-control.line.left, .react-flow__resize-control.line.right {
  width: 1px; transform: translate(-50%, 0); top: 0; height: 100%;
}
...
.react-flow__resize-control.line.top, .react-flow__resize-control.line.bottom {
  height: 1px; transform: translate(0, -50%); left: 0; width: 100%;
}
