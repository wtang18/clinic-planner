  ...
  if (action.type === 'SUGGESTION_ACCEPTED') {
    ...
    if (suggestion && suggestion.content.type === 'new-item') {
      ...
      const newItem: ChartItem = {
        ...
        source: { type: 'aiSuggestion', suggestionId: suggestion.id },
        tags: [],
        linkedDiagnoses: [],
        linkedEncounters: [],
        activityLog: [{
          ...
          action: 'created',
          actor: 'AI System',
          details: `Accepted from suggestion ${suggestion.id}`,
        }],
        _meta: { syncStatus: 'local', aiGenerated: true, requiresReview: false, reviewed: false },
