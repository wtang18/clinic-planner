/**
 * Generate React Native icon map with inlined SVG strings
 *
 ...
 * Or:  node src/design-system/icons/generate-rn-icon-map.js
 *
...
const SMALL_DIR = path.join(__dirname, '../icons/small');
const MEDIUM_DIR = path.join(__dirname, '../icons/medium');
const OUTPUT_FILE = path.join(__dirname, '../icon-map.rn.ts');
...
/**
 * Generate the React Native icon map file
 */
function generateRnIconMap() {
  ...
  // Generate map entries for small icons with inlined SVG content
  ...
  // Generate map entries for medium icons with inlined SVG content
  ...
  const content = `/**
 * Auto-generated React Native icon map with inlined SVG strings
...
try {
  ...
  console.error('❌ Error generating RN icon map:', error);
  process.exit(1);
}
